...
pub fn file_dialog() -> Option<String> {
    log_info("Executable dialog opened");
...
pub fn image_dialog() -> Option<String> {
    log_info("Image dialog opened");
    ...
    thread::spawn(move || {
        ...
        if let Ok(code) = child.wait() {
            if code.success() {
                ...
                log_info(&format!("Game ran for {} second(s)", final_time.as_secs()));
            } else if let Some(c) = code.code() {
                log_error(&format!("Game exited with error: {}", c));
            }
        } else {
